...
import { confirmTradeSchema } from '@/lib/schemas/trade'
...
export async function PUT(
  ...
  try {
    ...
    // Validate input
    const validatedData = confirmTradeSchema.parse({
      ...body,
      tradeId
    })

    // Confirm trade delivery
    const result = await confirmTrade(validatedData, session.user.id)
